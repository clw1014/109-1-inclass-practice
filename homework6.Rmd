...
pickmember=str_extract(memberIds,pattern = memberID)
member=str_extract_all(members,pattern = pickmember)
member$latestGPS=currentGPS
}
...
{
  pick_creator <-
    str_which(names(painting$DACatalog$MetaDesc), "Creator")
  creator_text <-
    unlist(painting$DACatalog$MetaDesc[pick_creator])
  allCreators <- str_replace(creator_text,"作者：","")
}
```
